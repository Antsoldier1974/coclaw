		...
		info = await page.evaluate(async () => {
			...
			for (const [botId, conn] of manager.__connections) {
				if (conn.state === 'connected' && conn.transportMode === 'rtc') {
					return { botId, transportMode: conn.transportMode };
				}
			}
			return null;
		});
...
test('附件发送：上传的文件实际存在于 agent workspace @chat @file', async ({ page }) => {
	...
	const downloaded = await page.evaluate(async ({ botId, filePath }) => {
		const { useBotConnections } = await import('/src/services/bot-connection-manager.js');
	...
	}, { botId: rtcInfo.botId, filePath: attachmentPath });
