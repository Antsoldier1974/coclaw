...
1. 服务端消息是否已包含该 run 的最终结果 — 检查 run.startTime 之后是否存在 assistant 消息，且最后一条 assistant 消息的 `stopReason` 为终止类型（`stop` / `maxTokens` / `endTurn`，而非 `toolUse`）
...
onAppForeground():
  if state === 'disconnected':
    ...
    if elapsed > ASSUME_DEAD_MS (45s):
      ...
      → probe(timeout: PROBE_TIMEOUT_MS=2.5s)
        成功 → 连接存活，无需操作
        超时 → forceReconnect()
```
...
| `STALE_RUN_MS` | 3000 | chatStore | 事件流静默判定阈值 |
