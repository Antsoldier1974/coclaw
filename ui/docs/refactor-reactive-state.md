...
### 1.2 BotConnectionManager 桥接

**现状**：BotConnectionManager 是纯 JS 类，管理 `Map<botId, BotConnection>` 的增删查。各消费者自行 `conn.on('state')` 桥接。

**目标**：BotConnectionManager 的 `connect()` 方法内注册**唯一**的 conn state 监听，将连接状态实时写入 `botsStore.byId[botId]`。
...
connect(botId, options) {
	...
	conn.on('state', (s) => {
		const bot = botsStore.byId[botId];
		if (!bot) return;
		bot.connState = s;
		if (s === 'disconnected') {
...
**lastAliveAt 实时同步**：BotConnection 每收到 WS 消息时更新 `__lastAliveAt`（在 `__resetHbTimeout` 中）。增加一行桥接即可：

```javascript
// BotConnection.__resetHbTimeout() 内或通过 BotConnectionManager 注册的回调
// 写入只是赋值一个数字字段，成本极低
bot.lastAliveAt = Date.now();
```

实现方式：在 BotConnectionManager 注册的 state 监听旁，再注册一个消息级回调（如 BotConnection emit 一个 `alive` 事件），或直接在 BotConnection.__resetHbTimeout 内通过回调写入。具体方式实施时确定，原则是让 store 中的 `lastAliveAt` 与 conn 实例保持实时一致。

这样 Capacitor 前台恢复也可以直接读 `botsStore.byId[id].lastAliveAt`，不必绕 conn 实例，保持消费路径一致。
...
| 文件 | 改动 |
|------|------|
| services/bot-connection-manager.js | connect() 内注册桥接 |
| services/bot-connection.js | 可能需要 emit alive 事件或提供回调注册点 |
| services/bot-connection-manager.test.js | 测试桥接 |

### 1.3 __listenForReady 改为响应式 watch

**现状**：`__listenForReady` 使用模块级 `_listeningConnIds` / `_initializedBots` Set 手动管理持久 state 监听，包含首次初始化和重连两个分支，逻辑重复。

**目标**：在 botsStore 内部设置 watch，监听 `byId` 中各 bot 的 `connState` 变化。当 connState 变为 `'connected'` 时触发对应的初始化/重连逻辑。视为该 store 对象的一个内部事件处理器。

```javascript
// stores/bots.store.js 内部
// 在 store 定义中通过 Pinia 的 setup 或 action 内启动 watch
function __watchConnStates() {
	watch(
		() => {
			const entries = [];
			for (const [id, bot] of Object.entries(this.byId)) {
				entries.push({ id, connState: bot.connState });
			}
			return entries;
		},
		(entries, prev) => {
			for (const { id, connState } of entries) {
				if (connState !== 'connected') continue;
				const was = prev?.find(p => p.id === id)?.connState;
				if (was === 'connected') continue;
...
`__onBotConnected(id)` 整合现有 `fire` + 重连分支逻辑。"首次 vs 重连"通过 `byId[id].initialized` 字段判断。

不需要 `_listeningConnIds` / `_initializedBots` Set——Vue watch 在同一 tick 内只触发一次，天然去重。

**涉及文件**：

| 文件 | 改动 |
|------|------|
| stores/bots.store.js | __listenForReady → __watchConnStates + __onBotConnected，移除模块级 Set |
...
| botsStore.items (deep) | 验证路由 | 保留（改为 items getter 或 byId） |
| agentsStore.byBot (deep) | 验证路由 | 保留 |
| isBotOffline | 上线时 loadMessages | **移除**，合并到 connReady |
...
阶段一（本次）：
  1.1 botsStore 结构重构（byId 聚合）
   ↓
  1.2 BotConnectionManager 桥接（conn state + lastAliveAt → botsStore.byId）
   ↓
  1.3 __listenForReady 改为 watch connState（内聚在 botsStore）
   ↓  可与 1.5 并行
  1.4 chatStore 去除连接监听 + ChatPage connReady watcher
   ↓
  1.5 sendMessage 重连等待改造

阶段二（后续）：chatStore 职责拆分
阶段三（后续）：事件桥接集中化 / BotConnectionManager 存废 / 重命名
```

阶段一内部依赖关系：1.1 → 1.2（桥接写入需要 byId 结构）→ 1.3 和 1.4（消费 connState）。1.5 与 1.3/1.4 无强依赖，可并行。

每步完成后运行完整测试（`pnpm check && pnpm test`）确保无回归。
