# CLI bind/unbind 与 Gateway 通信

> **2026-03-21 架构更新**：`openclaw coclaw bind/unbind` 已改为瘦 CLI，通过 `coclaw.bind`/`coclaw.unbind` gateway RPC 在 gateway 进程内执行全部逻辑（包括 bridge 管理）。CLI 不再直接操作 `bindings.json`，也不再需要二次 RPC 通知 `refreshBridge`/`stopBridge`。
>
> 本文档的"当前方案"部分描述的"CLI 本地执行 + RPC 通知"架构仅适用于遗留的独立 CLI（`src/cli.js`）。
...
### 注册 CLI 路径（`openclaw coclaw bind/unbind`）— 推荐

CLI 是瘦壳：参数解析 → `callGatewayMethod('coclaw.bind/coclaw.unbind')` → 展示结果。Gateway 的 RPC handler 内部执行 `doBind`/`doUnbind`，统一管理 stop bridge → unbind/bind → write config → start bridge 的完整流程。RPC 超时 30s。

### 斜杠命令路径（`/coclaw bind/unbind`）

运行在 Gateway 进程内，直接调用 `doBind`/`doUnbind`，与 RPC handler 共享同一内部函数。

### 遗留独立 CLI（`src/cli.js`）

直接在 CLI 进程中调用 `bindBot`/`unbindBot`，成功后通过 `coclaw.refreshBridge`/`coclaw.stopBridge` RPC 通知 gateway。此路径不具备瘦 CLI 的架构保证。
...
### 中间方案：Gateway RPC 通知 + spawn（2026-03-05，已被瘦 CLI 方案取代）
...
4. 总超时默认 10 秒（`NOTIFY_TIMEOUT_MS`）；注册 CLI 路径的 `coclaw.bind`/`coclaw.unbind`/`coclaw.enroll` 覆盖为 30 秒（`RPC_TIMEOUT_MS`）
...
| 失败 | `respond(false, undefined, { code, message })` | CLI 抛异常，非零退出码 |
