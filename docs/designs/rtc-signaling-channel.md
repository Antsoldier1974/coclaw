...
| full rebuild 耗尽 | 标记 failed → 退避重试（10s→120s 指数退避，最多 8 次）；外部事件（applySnapshot / foreground-resume / network:online）重置退避计数 |
...
| 3 | full rebuild 耗尽 | reject 挂起请求（`RTC_LOST`）→ 退避重试（10s→120s，最多 8 次），外部事件重置退避 |
