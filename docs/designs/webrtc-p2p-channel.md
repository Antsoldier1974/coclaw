...
> Phase 1：`rpc` 通道创建但不承载业务数据。Phase 2 启用 RPC 通信，详见 `webrtc-p2p-channel-phase2.md`。
...
__setupDataChannel(connId, dc) {
	...
	dc.onmessage = (event) => {
		// Phase 1：仅日志。Phase 2 改为解析 JSON 并分发到 onRequest 回调
...
> Phase 2 时需额外传入 `onRequest` 回调，并在 gateway 响应转发处追加 `broadcast()` 调用。
...
#### 9.2.1 BotConnection 适配（Phase 1 两处变更，Phase 2 扩展见 `webrtc-p2p-channel-phase2.md` §7.1.1）
...
async connect(turnCreds) {
	...
	dc.onmessage = (event) => {
		// Phase 1：仅日志。Phase 2 改为解析 JSON 并回调 BotConnection.__onRtcMessage
...
```ini
# 本地开发用，与 compose.dev.yaml 中 coturn 的 static-auth-secret 保持一致
TURN_SECRET=dev-secret-placeholder
APP_DOMAIN=localhost
```

#### coturn 容器（compose.dev.yaml）

`deploy/compose.dev.yaml` 已包含 coturn 服务，使用 `network_mode: host` 直接绑定宿主机网络栈：

```bash
cd deploy && docker compose -f compose.dev.yaml up -d coturn
```

**为什么本地开发也需要 coturn**：

在 WSL2 环境中，Browser 运行在 Windows，Plugin 运行在 WSL2，两者处于不同的网络命名空间。WSL2 仅自动转发 TCP（HTTP/WebSocket 正常），**不转发 UDP**（WebRTC ICE 连接检查依赖 UDP）。没有 TURN 中继时，ICE 连接无法建立。

coturn 解决此问题的原理：
- Browser 通过 TCP transport（`turn:localhost:3478?transport=tcp`）连接 coturn，WSL2 自动转发 TCP ✓
- Plugin 通过 UDP 直接连接 coturn（同在 WSL2 内）✓
- coturn 在两者之间中继数据，绕过 UDP 不通的问题

> 非 WSL2 环境（原生 Linux/macOS）下，UI 和 Plugin 在同一网络栈，ICE 会直接选择 `host` candidate 直连。coturn 仍可正常运行但不会被使用（ICE 优先选择延迟最低的路径）。

#### 联调验证要点

1. coturn 容器运行：`docker ps --filter name=coclaw-coturn`
2. TURN 凭证 API：`curl -b <session_cookie> http://localhost:3000/api/v1/turn/creds` 返回有效 JSON
3. 浏览器 DevTools Console 应出现完整日志链：
   ```
   [WebRTC] offer sent for bot <botId>
   [WebRTC] answer received, setting remote description
   [WebRTC] connectionState: connected
   [WebRTC] ICE connected via <host|srflx|relay> (P2P|TURN)
   [WebRTC] DataChannel "rpc" opened
   ```
...
详见 `webrtc-p2p-channel-phase2.md`。核心：将 JSON-RPC 消息迁移到 DataChannel，WS 作为兜底。
