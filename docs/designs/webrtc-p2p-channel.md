...
| 消息大小限制 | 受 SCTP `maxMessageSize` 约束（通常 256KB）。超过 64KB 的 payload 自动回退到 WS 发送（见§十五 TODO-1） |
...
1. **RPC DataChannel 大消息分片**：`rpc` DataChannel 不支持超过 SCTP `maxMessageSize` 的单条消息（如包含 base64 编码图片的 agent 请求）。当前临时方案：`bot-connection.js` 中对超过 64KB 的 payload 自动回退到 WS 发送（不改变 `transportMode`），DC 上的事件广播保证流式响应不丢失。长期应实现应用层分片/重组，使所有 RPC 消息均可走 DataChannel。
2. **大消息 WS 回退时的 `res` 双到达**：大消息走 WS 发出后，Plugin 会将 `res` 同时广播到 server WS 和 DataChannel，导致同一 `res` 在 UI 两条路径各到达一次。当前有 guard 保护不出错，但两阶段 `accepted` 回调可能被调用两次。低频场景，暂不处理。
